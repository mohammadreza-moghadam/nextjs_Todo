...
const ToDoCard = ({ toDo }) => {
  ...
  const deleteToDo = (e) => {
    ...
    if (window.confirm("آیا مطمین هستید؟")) {
  ...
  return (
    <div className={`todo ${toDo.complete ? "todo--complete" : ""}`}>
      ...
      <div className="todo__controls">
        {!editing ? (
          <>
            {!toDo.complete && <button onClick={onEdit}>ویرایش</button>}
            <button onClick={deleteToDo}>حذف</button>
          </>
        ) : (
          <>
            <button onClick={stopEditing}>لغو</button>
            <button onClick={editToDo}>ذخیره</button>
          </>
        )}
      </div>
    </div>
